...
module.exports = function(schema, options) {
    ...
    schema.methods.setPassword = function (password, cb) {
        ...
        crypto.randomBytes(options.saltlen, function(err, buf) {
            ...
            crypto.pbkdf2(password, salt, options.iterations, options.keylen, options.algorithm, function(err, hashRaw) {
                if (err) {
                    return cb(err);
                }
    ...
    schema.methods.authenticate = function(password, cb) {
        ...
        crypto.pbkdf2(password, this.get(options.saltField), options.iterations, options.keylen, options.algorithm, function(err, hashRaw) {
            if (err) {
                return cb(err);
            }
