...
module.exports = function(schema, options) {
  ...
  schema.methods.setPassword = function(password, cb) {
    if (!password) {
      return cb(new errors.MissingPasswordError());
    }
  ...
  function authenticate(user, password, cb) {
    if (options.limitAttempts) {
      ...
      if (Date.now() - user.get(options.lastLoginField) < calculatedInterval) {
        ...
        user.save();
        return cb(null, false, new errors.AttemptTooSoonError());
      }

      if (user.get(options.attemptsField) >= options.maxAttempts) {
        return cb(null, false, new errors.TooManyAttemptsError());
      }
    }

    if (!user.get(options.saltField)) {
      return cb(null, false, new errors.NoSaltValueStoredError());
    }
        ...
        if (options.limitAttempts) {
          ...
          user.save(function(saveErr) {
            ...
            if (user.get(options.attemptsField) >= options.maxAttempts) {
              ...
              return cb(null, false, new errors.IncorrectPasswordError());
            }
          });
        } else {
          return cb(null, false, new errors.IncorrectPasswordError());
    ...
    if (!self.get(options.saltField)) {
      self.constructor.findByUsername(self.get(options.usernameField), true, function(err, user) {
        ...
        if (user) {
          ...
          return cb(null, false, new errors.IncorrectUsernameError(options.usernameField));
        }
      });
    } else {
  ...
  schema.statics.register = function(user, password, cb) {
    ...
    if (!user.get(options.usernameField)) {
      return cb(new errors.MissingUsernameError(options.usernameField));
    }
    ...
    self.findByUsername(user.get(options.usernameField), function(err, existingUser) {
      ...
      if (existingUser) {
        return cb(new errors.UserExistsError(options.usernameField, user.get(options.usernameField)));
      }
